...
Astar::Astar(std::string &input_file, std::string &output_file, std::string _heuristic_function) {
    ...
    this->heuristic_function = std::move(_heuristic_function);
...
void Astar::AstarSearch() {
    ...
    while (!open_list.empty()) {
        ...
        for (auto &neighbor: neighbors) {
            ...
            if (heuristic_function == "non-trivial" && neighbor_h == -1) {
                continue;
            }
            if (!is_in_close_list && !is_in_open_list) {
                neighbor.setSupply(neighbor_supply);
                neighbor.setG(neighbor_g);
                neighbor.setH(neighbor_h);
                neighbor.setParentPos(curr_point.getPos());
                open_list.emplace(neighbor);
            } else if (is_in_open_list) {
                // 如果 neighbor_g < open list 中该点的 g 值，
                // 则更新 open_list中该点 g 值
                Point neighbor_in_open_list = *open_list_iter;
                if (neighbor_g < neighbor_in_open_list.getG()) {
                    neighbor_in_open_list.setG(neighbor_g);
                    neighbor_in_open_list.setH(neighbor_h);
                    neighbor_in_open_list.setParentPos(curr_point.getPos());
...
//                    std::cout << "neighbor_supply: " << neighbor_supply <<
//                            " neighbor.getSupply(): " << neighbor_in_close_list.getSupply() << std::endl;
                    neighbor_in_close_list.setSupply(neighbor_supply);
                    neighbor_in_close_list.setG(neighbor_g);
                    neighbor_in_close_list.setH(neighbor_h);
                    neighbor_in_close_list.setParentPos(curr_point.getPos());
...
int Astar::HeuristicFunction(Point &point, int curr_supply) {
    if (heuristic_function == "trivial") {
        if (curr_supply <= 0) {
            return -1;
        }
        return point.distance(map.getEnd());
    } else if (heuristic_function == "non-trivial") {
        /**
        ...
        */
        ...
        std::cout << R"(Error: heuristic function is not "trivial" or "non-trivial"!)" << std::endl;
        return -1;
    }
}
...
void Astar::GetResult() {
    ...
    if (status == AStarNotFound) {
        ...
        std::pair<int, int> parent_pos = it->getParentPos();
